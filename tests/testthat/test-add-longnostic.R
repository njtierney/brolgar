...
df_add_l_diff_1 <- features(wages_ts, ln_wages, diff)
df_add_n_key_obs <- add_n_key_obs(wages_ts)
df_add_key_slope <- add_key_slope(wages_ts, ln_wages ~ xp)
df_add_key_slope_multi <- add_key_slope(wages_ts, ln_wage ~ xp + ged)
...
test_that("longnostic returns correct classes", {
  expect_equal(classes(df_add_l_diff_1)[[".diff_1"]], "numeric")
