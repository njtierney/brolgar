...
wages_strat_along <- wages_test %>%
  stratify_keys(n_strata = 10, along = unemploy_rate, fun = median)
...
test_that(".strata is added to the dataframe", {
  expect_equal(names(wages_strat), c(names(wages_test), ".strata"))
  expect_equal(names(wages_strat_along), c(names(wages_test), ".strata"))
})
...
wages_strat_along_sum <- wages_strat_along %>%
  as_tibble() %>%
  group_by(.strata) %>%
  summarise_at(vars(unemploy_rate), list(mean = mean), na.rm = TRUE)
