...
test_that("`near_between()` works on a vector", {
  expect_equal(
    near_between(x, from = 0.4, to = 0.6),
    c(rep(FALSE, 4), TRUE, TRUE, rep(FALSE, 4))
  )
  expect_equal(
    near_between(x, from = 0.1, to = 0.3),
...
wages_feat_near_btn <- wages_feat %>%
  filter(near_between(x = min, from = 0.4, to = 0.6))
...
test_that("`near_middle()` works on a vector", {
  expect_equal(
    near_middle(x = x, middle = 0.5, within = 0.2),
    c(rep(FALSE, 4), TRUE, TRUE, rep(FALSE, 4))
  )
  expect_equal(
    near_middle(x = x, middle = 0.2, within = 0.1),
    c(FALSE, FALSE, TRUE, rep(FALSE, 7))
  )
})

wages_feat_near_middle <- wages_feat %>%
  filter(near_middle(x = min, middle = 0.5, within = 0.2))
...
test_that("`near_quantile()` works on a vector", {
  expect_equal(near_quantile(x, probs = 0.5, tol = 0.01), rep(FALSE, 10))
  expect_equal(
    near_quantile(x, probs = 0.5, tol = 0.5),
    c(rep(FALSE, 4), TRUE, TRUE, rep(FALSE, 4))
  )
  expect_equal(near_quantile(x, probs = 0.25, tol = 0.01), rep(FALSE, 10))
  expect_equal(
    near_quantile(x, probs = 0.25, tol = 0.5),
    c(rep(FALSE, 2), TRUE, rep(FALSE, 7))
  )
  expect_equal(
    near_quantile(x, probs = c(0.25, 0.75), tol = 0.01),
    rep(FALSE, 10)
  )
  expect_equal(
    near_quantile(x, probs = c(0.25, 0.75), tol = 0.5),
...
test_that("`nearest_qt_lgl()` works for vectors", {
  expect_equal(nearest_qt_lgl(x, c(0.5)), c(rep(FALSE, 4), TRUE, rep(FALSE, 5)))
  expect_equal(sum(nearest_qt_lgl(x, c(0.5))), 1)
  expect_equal(
    nearest_qt_lgl(x, c(0.25, 0.5, 0.75)),
    c(FALSE, FALSE, TRUE, FALSE, TRUE, FALSE, FALSE, TRUE, FALSE, FALSE)
  )
  expect_equal(sum(nearest_qt_lgl(x, c(0.25, 0.5, 0.75))), 3)
  expect_equal(
    nearest_qt_lgl(x, c(0.1, 0.5, 0.9)),
    c(FALSE, TRUE, FALSE, FALSE, TRUE, rep(FALSE, 3), TRUE, FALSE)
  )
