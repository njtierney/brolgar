...
test_that("qtl works", {
  expect_equal(
    as.numeric(qtl(q_vec, 0.25)),
    quantile(q_vec, 0.25, names = FALSE)
  )

  expect_equal(names(qtl(q_vec, 0.25)), "q_25")

  expect_equal(
    as.numeric(qtl(q_vec, c(0.25, 0.50))),
    quantile(q_vec, c(0.25, 0.50), names = FALSE)
  )

  expect_equal(names(qtl(q_vec, c(0.25, 0.50))), c("q_25", "q_50"))

  expect_equal(
    as.numeric(qtl(q_vec, seq(0.25, 0.5, by = 0.05))),
    quantile(q_vec, seq(0.25, 0.5, by = 0.05), names = FALSE)
  )

  expect_equal(
    names(qtl(q_vec, seq(0.25, 0.5, by = 0.05))),
...
test_that("test_if_null returns different error messages", {
  ...
  expect_error(
    test_if_null(NULL, message = "wat even is this"),
    regexp = "even is this"
  )
})
