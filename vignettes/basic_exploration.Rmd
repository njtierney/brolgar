...
We can kind the number of keys using `n_obs()`:
...
Note that this is a single number, in this case, we have `r n_obs(wages_ts)` observations.

However, we might want to know how many observations we have for each individual. If we want the number of observations in each key (id), then we can use `l_n_obs`. This is a special function from `brolgar`, indicated by `l_`. Every `l_` function will return us a dataframe.
...
wages_ts %>%
  group_by_key() %>%
  summarise(mean_time = mean(exper))
```
