...
We can reduce data down to the school level, within each country - and in doing so, it is important that we provide some summary statistics of each of the scores - we want to include

```{r pisa-reduce}
pisa_school <- pisa %>% 
  group_by(year, country, school_id) %>%
...
```{r pisa-as-tsibble}
as_tsibble(pisa_school,
           key = c(country, school_id),
           index = year,
           regular = TRUE)
```
...
And from what we can see, it looks like `student_id` is not duplicated - let's try arranging it.

```{r extra-pisa-duplicates}
duplicates(pisa, 
           key = c(country, 
                   school_id, 
                   student_id), 
